...
def create_employer_details_example_file(content)
  path = "generated/broker_1/employer_details_#{@details_example_no}.json"
...
def create_employee_roster_example_file(content)
  path = "generated/broker_1/roster_#{@roster_example_no}.json"
  File.write("#{__dir__}/#{path}", JSON.pretty_generate(content))
  @roster_example_no += 1
  "https://raw.githubusercontent.com/dchealthlink/HBX-mobile-app-APIs/#{@branch}/#{path}"
end
...
min_months_to_renew = if now.mday > 10 then
                        2
                      else
                        1
                      end
min_months_to_enroll = if now.mday > 13 then
                         2
                       else
                         1
                       end
...
path = 'generated/broker_1/broker_details.json'
File.write("#{__dir__}/#{path}", JSON.pretty_generate(result))
